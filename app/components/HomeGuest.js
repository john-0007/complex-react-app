		...
		switch (action.type) {
			...
			case 'usernameAfterDelay':
				if (draft.username.value.length < 3) {
					draft.username.hasErrors = true
					draft.username.message = 'Username must be at least 3 charecters'
				}
			...
			case 'usernameUiqueResults':
				console.log('dd', action.value)
				if (action.value) {
					draft.username.hasErrors = true
		...
		if (state.submitCount) {
			...
			async function fetchResult() {
				try {
					...
					appDispatch({ type: 'flashMessage', value: 'Congrats! Welcome ot your new account.' })
				} catch (e) {
					console.log('There was a problem.')
				}
			}
